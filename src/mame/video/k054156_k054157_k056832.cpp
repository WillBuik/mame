...
static bool known_off_y = false;

static int off_for_y[][8] = {
	{ 6000000, 2, 15, 224, 17, 8, 1 },   // gokuparo
	{ 8000000, 2, 15, 256, 12, 6, 1 },   // racinfrc
	{ 8000000, 2, 15, 224, 17, 8, 1 },   // dragoona
	{ 8000000, 1, 16, 224, 16, 8, 1 },   // piratesh 0?
	{ 6000000, 1, 15, 224, 17, 8, 1 },   // mystwarr, mmaulers (mmaulers wants 2?)
	{ 6000000, 0, 16, 224, 16, 8, 1 },   // metamrph
	{ 8000000, 0, 15, 256, 12, 6, 1 },   // xexex
	{ }
};
...
void k054156_056832_device::ksnotifier_w(int clk, int hbp, int hv, int hfp, int hs, int vbp, int vv, int vfp, int vs)
{
	clk_ = clk;
	...
	logerror("notifier %d %d %d %d %d %d %d %d %d\n", clk, hbp, hv, hfp, hs, vbp, vv, vfp, vs);
	known_off_x = false;
	m_global_offx = 0;
	...
	known_off_y = false;
	m_global_offy = 0;
	for(unsigned int i=0; off_for_y[i][0]; i++)
		if(off_for_y[i][0] == clk && off_for_y[i][1] == typ_ && off_for_y[i][2] == vbp && off_for_y[i][3] == vv && off_for_y[i][4] == vfp && off_for_y[i][5] == vs) {
			m_global_offy = off_for_y[i][6];
			known_off_y = true;
			break;
		}
	if(!known_off_y)
		logerror("offset unknown for Y: %d %d %d %d %d %d\n", clk, typ_, vbp, vv, vfp, vs);
}
		...
		if(tick == 5) {
			...
			if(!known_off_y) {
				...
				if(machine().input().code_pressed(KEYCODE_G))
					m_global_offy--;
				if(o != m_global_offy || machine().input().code_pressed(KEYCODE_V))
					logerror("new Y offset: { %d, %d, %d, %d, %d, %d, %d }\n", clk_, typ_, vbp_, vv_, vfp_, vs_, m_global_offy);
			}
		}
	}
