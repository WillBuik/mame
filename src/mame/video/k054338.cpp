...
WRITE16_MEMBER( k054338_device::bri1_w)
{
	if(ACCESSING_BITS_0_7) {
		if((data & 0xff) != m_brightness[0])
			logerror("brightness 1 %02x %s\n", data & 0xff, machine().describe_context());
		if((data & 0xff) == 0xff && m_brightness[0] == 0xfc)
			machine().debug_break();
...
void k054338_device::flow_render_register_renderers()
{
	m_renderer = flow_render_create_renderer([this](const rectangle &cliprect){ render(cliprect); });
	m_renderer_input_color[0] = m_renderer->create_input_sb_u16("0 color");
	m_renderer_input_attr[0]  = m_renderer->create_input_sb_u16("0 attr");
	m_renderer_input_color[1] = m_renderer->create_input_sb_u16("1 color");
	m_renderer_input_attr[1]  = m_renderer->create_input_sb_u16("1 attr");
	m_renderer_output = m_renderer->create_output_sb_rgb();
}

void k054338_device::render(const rectangle &cliprect)
{
	const uint8_t *shadow_wrap = (m_system & 0x20 ? m_through_shadow_table : m_clip_shadow_table) + 0x100;
	const uint32_t *pens = m_palette->pens();
	uint16_t mask = m_palette->entries() - 1;
	...
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		const uint16_t *b0color = &m_renderer_input_color[0]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *b0attr  = &m_renderer_input_attr[0]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *b1color = &m_renderer_input_color[1]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *b1attr  = &m_renderer_input_attr[1]->bitmap().pix16(y, cliprect.min_x);
		uint32_t *dest = &m_renderer_output->bitmap().pix32(y, cliprect.min_x);
		for(int x = cliprect.min_x; x <= cliprect.max_x; x++) {
			uint16_t b0c = line_mapper(*b0color++);
			uint16_t b0a = *b0attr++;
			uint16_t b1c = line_mapper(*b1color++);
