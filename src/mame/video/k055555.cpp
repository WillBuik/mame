...
void k055555_device::flow_render_register_renderers()
{
	m_renderer = flow_render_create_renderer([this](const rectangle &cliprect){ render(cliprect); });

	m_renderer_input_simple_color[0]  = m_renderer->create_input_sb_u16("a color");
	m_renderer_input_simple_color[1]  = m_renderer->create_input_sb_u16("b color");
	m_renderer_input_simple_color[2]  = m_renderer->create_input_sb_u16("c color");
	m_renderer_input_simple_color[3]  = m_renderer->create_input_sb_u16("d color");
	m_renderer_input_complex_color[0] = m_renderer->create_input_sb_u16("o color");
	m_renderer_input_complex_attr[0]  = m_renderer->create_input_sb_u16("o attr");
	m_renderer_input_complex_color[1] = m_renderer->create_input_sb_u16("s1 color");
	m_renderer_input_complex_attr[1]  = m_renderer->create_input_sb_u16("s1 attr");
	m_renderer_input_complex_color[2] = m_renderer->create_input_sb_u16("s2 color");
	m_renderer_input_complex_attr[2]  = m_renderer->create_input_sb_u16("s2 attr");
	m_renderer_input_complex_color[3] = m_renderer->create_input_sb_u16("s3 color");
	m_renderer_input_complex_attr[3]  = m_renderer->create_input_sb_u16("s3 attr");

	m_renderer_output_color[0] = m_renderer->create_output_sb_u16("0 color");
	m_renderer_output_attr[0]  = m_renderer->create_output_sb_u16("0 attr");
	m_renderer_output_color[1] = m_renderer->create_output_sb_u16("1 color");
	m_renderer_output_attr[1]  = m_renderer->create_output_sb_u16("1 attr");
}

void k055555_device::render(const rectangle &cliprect)
{
	uint16_t prixor = m_bgc_set & 8 ? 0x7ff : 0;
	uint8_t os_inbri_masked = m_os_inbri & m_os_inbri_on;
	uint8_t v_inmix_masked  = m_v_inmix  & m_v_inmix_on;
	...
	//	logerror("%x %02x %02x %02x / %02x %02x\n", m_colchg_on, m_colpri[0], m_pri[0], m_pri[8], m_pri[1], m_pri[9]);
	for(int y = cliprect.min_y; y <= cliprect.max_y; y++) {
		const uint16_t *acolor  = &m_renderer_input_simple_color[0] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *bcolor  = &m_renderer_input_simple_color[1] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *ccolor  = &m_renderer_input_simple_color[2] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *dcolor  = &m_renderer_input_simple_color[3] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *ocolor  = &m_renderer_input_complex_color[0]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *oattr   = &m_renderer_input_complex_attr[0] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s1color = &m_renderer_input_complex_color[1]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s1attr  = &m_renderer_input_complex_attr[1] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s2color = &m_renderer_input_complex_color[2]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s2attr  = &m_renderer_input_complex_attr[2] ->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s3color = &m_renderer_input_complex_color[3]->bitmap().pix16(y, cliprect.min_x);
		const uint16_t *s3attr  = &m_renderer_input_complex_attr[3] ->bitmap().pix16(y, cliprect.min_x);
		uint16_t *fc = &m_renderer_output_color[0]->bitmap().pix16(y, cliprect.min_x);
		uint16_t *fa = &m_renderer_output_attr[0] ->bitmap().pix16(y, cliprect.min_x);
		uint16_t *bc = &m_renderer_output_color[1]->bitmap().pix16(y, cliprect.min_x);
		uint16_t *ba = &m_renderer_output_attr[1] ->bitmap().pix16(y, cliprect.min_x);
		for(int x = cliprect.min_x; x <= cliprect.max_x; x++) {
			uint16_t prif = 0x7ff, prib = 0x7ff;
			uint16_t colorf = 0x0000, colorb = 0x0000, attrf = 0x0000, attrb = 0x0000;
			uint16_t oa = *oattr++;
			...
			if(disp & 0x01) {
				uint16_t col = *acolor++ & 0x3ff;
				if(col & m_color_mask[0]) {
					uint16_t lpri = (m_colchg_on & 8) && ((m_colpri[0] & m_color_mask[0]) <= (col & m_color_mask[0])) ? m_pri[8] : m_pri[0];
					lpri = ((lpri << 3) | 0) ^ prixor;
					prif = lpri;
					colorf = (col & (0x0ff | (m_v_inmix_on << 8))) | (m_cblk[0] << 10);
					attrf = m_shd_on & 0x01 ? 0x8001 : 0x8000;
			...
			if(disp & 0x02) {
				uint16_t col = *bcolor++ & 0x3ff;
				if(col & m_color_mask[1]) {
					uint16_t lpri = (m_colchg_on & 2) && ((m_colpri[1] & m_color_mask[1]) <= (col & m_color_mask[1])) ? m_pri[9] : m_pri[1];
					lpri = ((lpri << 3) | 1) ^ prixor;
